...
class QAOptimizer(object):
    ...
    def calculate_valid_loss(self, samples ,encoder,decoder):
        ...
        for i in tqdm(range(len(samples))):
            ...
            emb_ = emb_sum / count
            #scores_rel += self.predict_sentence(question,chain)[0]
    ...
    def train(self, loader, epoch,preheat=2):
        #### FIRST STAGE
        ...
        for i_batch, a in enumerate(loader):
            ...
            _,embeddings = self.generate_tail_embed(question, path)

            loss = self.calculate_qa_loss(question, embeddings, head, tail, question_param)
