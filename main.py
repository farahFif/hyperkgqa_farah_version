...
parser.add_argument(
    "--patience", default=1, type=int, help="Number of epochs before early stopping"
)
...
parser.add_argument(
    "--learning_rate", default=0.001, type=float, help="Learning rate for KGQA"
)
...
parser.add_argument(
    '--qa_nn_type', default='LSTM', choices=['ENC_DEC', 'LSTM', 'SBERT']
)
        ...
        for phase in phases:
            ...
            if phase == 'train':
                ...
                if score > best_score + eps and epoch < args.max_epochs:
                    ...
                    test_score, tops = optimizer.calculate_valid_loss(test_samples)
                    # print(' Test accuracy ',test_score)
                    # print('POST PROCESSING')
                    # optimizer.post_processing_pos_scores(test_samples)
...
def evaluate_with_relation_matching(dataset_path, qa_optimizer, test_samples, pruning_model, dataset, entity2idx,
                                    rel2idx):
...
def train_relation_matching_model(args, qa_dataset_path, device, rel2idx, word2idx, vocab_size,
    ...
    data_loader = getattr(dataloaders, 'DataLoaderPruning_{}'.format(args.qa_nn_type))(dataset,
                                                                                       batch_size=args.batch_size,
                                                                                       shuffle=True, num_workers=15)
    model = (getattr(pruning_models, '{}_PruningModel'.format(args.qa_nn_type))(args, rel2idx, vocab_size,
                                                                                pretrained_language_model)).to(device)
    ...
    train(optimizer, model, data_loader, scheduler, train_samples, valid_samples, test_samples, args,
          checkpoint_prm_path)
...
if __name__ == "__main__":
    ...
    data_loader = getattr(dataloaders, 'DataLoader_{}'.format(args.qa_nn_type))(dataset, batch_size=args.batch_size,
                                                                                shuffle=True, num_workers=20)

    #valid_dataset = getattr(dataloaders, 'Dataset_{}'.format(args.qa_nn_type))(valid_samples, word2idx, entity2idx, rel2idx)
    #valid_loader = getattr(dataloaders, 'DataLoader_{}'.format(args.qa_nn_type))(valid_dataset, batch_size=args.batch_size,
                                                                              #  shuffle=True, num_workers=20)

    ## Creat QA model
    print('Creating QA model')
    #qa_model = getattr(qamodels, '{}_QAmodel'.format(args.qa_nn_type))(args, embed_model, vocab_size, rel2idx,entity2idx,device).to(device)
    ...
    train(qa_optimizer, qa_model, data_loader, scheduler, train_samples, valid_samples, test_samples, args,
          checkpoint_path)


    if args.use_relation_matching:
        qa_optimizer.model = torch.load(checkpoint_path).to(device)
        checkpoint_prm_path, dataset = train_relation_matching_model(args, qa_dataset_path, device, rel2idx, word2idx,
                                                                     vocab_size, qa_optimizer.model.ln_model)
