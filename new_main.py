...
from qamodels.enc_dec_model import Encoder,Decoder
...
parser.add_argument(
    '--qa_nn_type', default='ENC_DEC', choices=['ENC_DEC','LSTM', 'SBERT']
)
...
def train(optimizer, model, data_loader, scheduler, train_samples, valid_samples, test_samples, args, checkpoint_path):
    ...
    def train_transformer():
        ...
        decoder = Decoder(args,model.rel2idx).to(device)
        optimizin  = torch.optim.Adam(list(encoder.parameters()) + list(decoder.parameters()),
                           lr=0.0001)
        ...
        for i in range(1):
            ...
            for i_batch, a in enumerate(data_loader):
                inp = a[0]
                question_param = a[1].to(device)
                path = a[4].to(device)
                loss = 0
                hidden = encoder(inp).to(device)
                dec_input = torch.tensor([[model.rel2idx['<start>']]] * len(a[0])).to(device)
                for t in range(1, path.shape[1]):
                    predictions, emb = decoder(dec_input, hidden)
                    loss += loss_function(path[:, t].long(), predictions)
                    dec_input = path[:, t].unsqueeze(1)

                batch_loss = (loss / int(path.size(1)))
                total_loss += batch_loss
                optimizin.zero_grad()
                loss.backward()
                optimizin.step()
                if i_batch % 100 == 0:
                    print('Epoch {} Batch {} Loss {:.4f}'.format(i + 1,
                                                                 i_batch,
    ...
    no_update = 0
    eps = 0.0001
    encoder , decoder = train_transformer()
    for epoch in range(args.max_epochs):
        for phase in phases:
            ...
            if phase == 'train':
                ...
                score = optimizer.train(loader, encoder,decoder,epoch)
                scheduler.step()
            ...
            if phase=='valid':
                ...
                decoder.eval()
                encoder.eval()
                score, _ = optimizer.calculate_valid_loss(valid_samples,encoder,decoder)
                if score > best_score + eps and epoch < args.max_epochs:
                    best_score = score
                    no_update = 0
                    print("Validation accuracy increased from previous epoch", score)
                    test_score, _ = optimizer.calculate_valid_loss(test_samples,encoder,decoder)
...
if __name__ == "__main__":
    ...
    qa_model = getattr(qamodels, '{}_QAmodel'.format(args.qa_nn_type))(args, embed_model, vocab_size,rel2idx,device).to(device)
    ## Create QA optimizer
